...
def predict_sample(model, batch, device, seed, settings):
    ...
    # Soft composite: soften the paste-back mask EDGE inward only — never spill
    # outside the original binary. Avoids leaking model output into face/hair
    # pixels adjacent to the mask (stray pixels in the mask can bleed over eyes).
    from torchvision.transforms.functional import gaussian_blur as _gb_final
    _M_blur = _gb_final(M.float(), kernel_size=[7, 7], sigma=2.0).to(M.dtype)
    _M_soft = _M_blur                                                          # outward: match exp524 panel/ composite (cleaner fabric)
