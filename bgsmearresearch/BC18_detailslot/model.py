...
class GarmentAdapter(nn.Module):
    ...
    def xattn(self, H, G, M):
        ...
        # ENH_DELTA_CLAMP>0: hard-bound |delta| via tanh so the enhancer can't run delta away to
        # win the branch-credit (unbounded delta blows up bg/skin + NaNs). Cap ~ a few units.
        _clamp = float(os.environ.get("ENH_DELTA_CLAMP", "0"))
        if _clamp > 0:
            delta = _clamp * torch.tanh(delta / _clamp)
