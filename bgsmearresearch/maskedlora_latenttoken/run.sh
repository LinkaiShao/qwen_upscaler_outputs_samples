...
export RUN_NAME="${RUN_NAME:-RMLORA_LATENTTOKEN}"
...
export USE_GARMENT_ADAPTER=1 GARMENT_ADAPTER_MODE=latent_token GARMENT_ADAPTER_LR=1e-4 
...
# ══════════ MASKED-LoRA OVERRIDE — SPATIAL LoRA GATING (image losses ON) ══════════
# Runs 2/3 had a REAL garment signal (correct beats wrong) but the GLOBAL LoRA leaked into
# bg/face -> halos. Fix: physically quarantine the LoRA delta to the garment region:
#   X_new = X + ΔW(X) * M ,  M = warped-mask C-slot garment token gate (1 on garment, 0 on
#   face/hands/bg + conditioning slots). Outside M, ΔW == 0 (LoRA forbidden from touching bg/face);
#   inside M, full capacity to learn identity. (masked_lora.py wraps each PEFT LoRA Linear.)
# Co-train the now-quarantined LoRA (FREEZE_LORA=0) + Run3's latent_token adapter, WITH image
# losses ON (halo is a PIXEL artifact — must decode to supervise it). Schedule + starvation kept.
export USE_MASKED_LORA=1 MASKED_LORA_DEBUG=1
export FREEZE_LORA=0 FREEZE_V6=1 USE_PCGRAD=0
# arch = Run3 latent_token adapter + 50/30/10/10 schedule + asymmetric starvation of std garment slot
export USE_GARMENT_ADAPTER=1 GARMENT_ADAPTER_MODE=latent_token GARMENT_ADAPTER_LR=1e-4
export GARMENT_SCHEDULE=1 GARMENT_SLOT_CORRUPT=zero GARMENT_SLOT_CORRUPT_ROUGH=1
export POSE_USE_WARPED_RGB=1
# ── IMAGE LOSSES ON (undo Run3's PURE_LATENT): decode the prediction so bg/face halo is supervised ──
export PURE_LATENT=0
export IMG_LOSS_WEIGHT=0.5 USE_V6_IMG_STAGE=1 LAMBDA_V6_REPAIR=0.8 RPCT_IMG_W=0.5 LAMBDA_V6_ROUTE=0.5
export W_IMG_V6_G=4.0 W_IMG_V6_G_ID=4.0 W_IMG_V6_G_STRUCT=0.3
export W_IMG_V6_S=3.0 W_IMG_V6_B=8.0 W_IMG_V6_OTHER=0.7 W_IMG_V6_K=0.3
export W_IMG_V6_UB=1.2 W_IMG_V6_UB_LO=0.3 W_IMG_V6_UB_HI=1.2
export W_GARMENT_EDGE_MATCH=8.0 GARMENT_EDGE_RING_PX=15 LAMBDA_LATE_SHELL=0 LAMBDA_BG_SHELL_AB=1.0
# ── in-loop DEPLOYED halo metric (watch bg/face halo directly during training) ──
export DEPLOY_HALO_EVAL=1 DEPLOY_EVAL_HOURS=0.6 DEPLOY_EVAL_STEPS=20
# ── batch 1 FORCED: image-loss VAE decode on cuda:1 (contended 31GB) OOMs at batch 2 (an earlier
#    inherited line defaulted BATCH_SIZE=2); PCGrad off so grad_accum=1. Edit here to change. ──
export BATCH_SIZE=1 GRAD_ACCUM=1
export TIME_BUDGET=${TIME_BUDGET:-7200}
export USE_FULL_TRAIN=1 TRAIN_LIMIT=0; unset OVERFIT_SIDS
# ════════════════════════════════════════════════════════════════

mkdir -p logs runs
RESDIR="bgsmearresearch/maskedlora_latenttoken"; mkdir -p "$RESDIR"; LOG="${ADAPTER_LOG:-$RESDIR/${RUN_NAME}.log}"
