...
export USE_GARMENT_KV="${USE_GARMENT_KV:-0}" USE_POSE_RESIDUAL=0 USE_LATE_XATTN=0 USE_GARMENT_XATTN=0
...
export USE_GARMENT_ADAPTER="${USE_GARMENT_ADAPTER:-1}" GARMENT_ADAPTER_MODE=state_enhancer GARMENT_ADAPTER_LR="${GARMENT_ADAPTER_LR:-5e-5}"   # env-overridable (was clobbering the BC block)
...
export PURE_LATENT="${PURE_LATENT:-1}"                    # env-overridable: BC24 sets 0 to enable the VAE decode (pred_img) for the perceptual loss
# image losses env-overridable (co-train needs them ON — pure-latent co-train blows up bg/skin)
export IMG_LOSS_WEIGHT="${IMG_LOSS_WEIGHT:-0}" USE_V6_IMG_STAGE="${USE_V6_IMG_STAGE:-0}" LAMBDA_V6_REPAIR="${LAMBDA_V6_REPAIR:-0}" RPCT_IMG_W="${RPCT_IMG_W:-0}" LAMBDA_PERCEPTUAL=0
export W_IMG_V6_G="${W_IMG_V6_G:-0}" W_IMG_V6_G_ID="${W_IMG_V6_G_ID:-0}" W_IMG_V6_G_STRUCT="${W_IMG_V6_G_STRUCT:-0}" W_IMG_V6_S="${W_IMG_V6_S:-0}" W_IMG_V6_B="${W_IMG_V6_B:-0}" W_IMG_V6_OTHER="${W_IMG_V6_OTHER:-0}" W_IMG_V6_K="${W_IMG_V6_K:-0}" W_IMG_V6_UB="${W_IMG_V6_UB:-0}"
...
export USE_GARMENT_ADAPTER="${USE_GARMENT_ADAPTER:-1}" GARMENT_ADAPTER_MODE="${BC_MODE:-state_enhancer}" GARMENT_ADAPTER_LR="${GARMENT_ADAPTER_LR:-5e-5}"
...
export OVERFIT_SIDS="00006_00,00008_00,00013_00,00017_00,00034_00"
# BC_MULTI=1: the DEFINITIVE test — train on ALL garments (unset the 5-img overfit) so the enhancer
# CANNOT memorize per-image additive hacks and MUST learn to READ the garment. The 5 reserved stay as val.
if [ "${BC_MULTI:-0}" = "1" ]; then unset OVERFIT_SIDS; echo "[BC_MULTI] MULTI-GARMENT: OVERFIT_SIDS unset, training on full dataset"; fi
export USE_FULL_TRAIN=1 TRAIN_LIMIT="${TRAIN_LIMIT:-0}"
export VAL_USE_RESERVED=1 VAL_LIMIT=5
