...
export USE_GARMENT_ADAPTER=1 GARMENT_ADAPTER_MODE=state_enhancer GARMENT_ADAPTER_LR=5e-5   # 5e-5 (was 1e-4: NaN@104 unclipped); + QK-norm + grad-clip now
...
# ══════════════════════════════════════════════════════════════════════════════
export RUN_NAME="${RN:-${RUN_NAME:-RUN_ENH_V2}}"          # honor RN=... alias; default RUN_ENH_V2
# state_enhancer at block 52 (reader block; frozen 0-51 LoRA, trainable 52-59)
export USE_GARMENT_ADAPTER=1 GARMENT_ADAPTER_MODE=state_enhancer GARMENT_ADAPTER_LR=5e-5
export GARMENT_ADAPTER_BLOCKS="${ENH_BLOCK:-52}" GARMENT_ADAPTER_GNET_BLOCKS=2
# co-train 52-59 LoRA at args.lr, FREEZE 0-51 via LR mult 0.0 (verify: "0..51 @ 0.00e+00 ... 52..59 @ <lr>")
export FREEZE_LORA=0 FREEZE_V6=1 USE_PCGRAD=0
export EARLY_BLOCK_CUTOFF=51 LR_EARLY_MULT=0.0
# READER LR: blocks 0-51 frozen (mult 0), so raising base LR only speeds the 52-59 reader LoRA.
# 5e-5 (10x run37's 5e-6) so the reader can actually LEARN to consume garment feats in a 1h overfit
# (avoids a false-negative "inert" from too-slow reader learning). Only 52-59 (12.5M) are trainable.
export LR=5e-5
unset LATE_BLOCK_CUTOFF LR_LATE_MULT                      # single mid tier (52..59) at args.lr
# (A) schedule 60/25/10/5 + mid/high-sigma starvation (kills GT-garment leak in C_t)
export GARMENT_SCHEDULE=1 ENH_SCHED_V2=1 GARMENT_SLOT_CORRUPT=zero GARMENT_SLOT_CORRUPT_ROUGH=1
export STARVE_HIGH_SIGMA=1 STARVE_SIGMA_LO=0.5
# (B) attention quarantine on blocks 53-59
export USE_ENH_QUARANTINE=1 ENH_QUARANTINE_BLOCKS="53,54,55,56,57,58,59"
