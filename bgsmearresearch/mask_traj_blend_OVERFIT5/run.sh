...
export RUN_NAME="${RUN_NAME:-MASKTRAJ_$(date +%H%M%S)}"
...
export LR=2e-5 LR_SCHEDULE=cosine LR_WARMUP=50 LR_COSINE_TOTAL=40000 LR_MIN_FRAC=0.1
...
# ── DISABLE every other garment path (only the mask-traj velocity denoiser is on) ──
export USE_GARMENT_KV=0 USE_POSE_RESIDUAL=0 USE_GARMENT_OOTD=0 USE_LATE_XATTN=0 USE_GARMENT_XATTN=0
...
# ══════════ MASKED TRAJECTORY BLENDING (user's #1 design) ══════════
#  v_final = v_run37*(1-M) + v_garment*M at EVERY step. run37 FULLY FROZEN (LoRA+v6+base);
#  train ONLY the garment velocity denoiser, and only through the warped-mask region M.
#  bg/skin are LOCKED by construction (v_final==v_run37 there). Clean diagnostic.
export USE_MASK_TRAJ_BLEND=1 MASK_TRAJ_BLOCKS=4 MASK_TRAJ_LR=1e-4
export POSE_USE_WARPED_RGB=1                         # loads warped_garment_rgb into the batch (denoiser input)
export FREEZE_LORA=1 FREEZE_V6=1 USE_PCGRAD=0        # run37 fully frozen; no shared params -> no PCGrad
# ── pure-latent: single-step flow MSE on v_final; skip the per-step VAE decode (fast) ──
...
export W_GARMENT_EDGE_MATCH=0 LAMBDA_LATE_SHELL=0 LAMBDA_BG_SHELL_AB=0 DEPLOY_HALO_EVAL=0
# ── deploy diagnostic envs (render passes these) ──
export MASK_TRAJ_DEPLOY_WRONG="${MASK_TRAJ_DEPLOY_WRONG:-0}" MASK_TRAJ_ZERO="${MASK_TRAJ_ZERO:-0}"
export TIME_BUDGET=${TIME_BUDGET:-3600}
export DEPLOY_EVAL_HOURS=0.6
# ════════════════════════════════════════════════════════════════

# ALL outputs under the run folder (NOTHING in /tmp)
RESDIR="bgsmearresearch/mask_traj_blend_OVERFIT5"
mkdir -p logs runs "$RESDIR"
LOG="${MASK_TRAJ_LOG:-$RESDIR/${RUN_NAME}.log}"
echo "RUN_NAME=${RUN_NAME}  LOG=${LOG}"
/home/link/venvs/ootd/bin/python -u train.py > "${LOG}" 2>&1
echo "train.py exited rc=$? RUN_NAME=${RUN_NAME} LOG=${LOG}"
