...
export RUN_NAME="${RUN_NAME:-COTRAIN_$(date +%H%M%S)}"
...
export USE_GARMENT_OOTD=1
export GARMENT_OOTD_LAYERS=4 GARMENT_OOTD_COPY_BLOCKS="36,44,50,55"
export GARMENT_OOTD_INJECT_BLOCKS="36,44,50,55"
export GARMENT_OOTD_DEPTH_SPECIFIC=1
export GARMENT_OOTD_GATE_SOURCE=warped_mask       # deploy-available per-token gate (NOT GT)
export GARMENT_OOTD_VG_ZERO_INIT=1                # to_v_g=0 at init -> step0 == run37 exactly
export GARMENT_OOTD_GATE_INIT_LOGIT=-2.0 GARMENT_OOTD_SIGMA_GATE=0
export GARMENT_OOTD_LR_BLOCKS=3e-6 GARMENT_OOTD_LR_AUX=3e-5 GARMENT_OOTD_LR_INJ=3e-5 GARMENT_OOTD_LR_GATE=1e-3
# ── Co-train: LoRA TRAINABLE, v6 frozen, PCGrad+maskgate = run37's own stability ──
export FREEZE_LORA=0 FREEZE_V6=1 USE_PCGRAD=1
# ── run69 DESTABILIZERS REMOVED: no CFG dropout, no garment-slot corruption ──
export CFG_DROPOUT=0.0
unset GARMENT_SLOT_CORRUPT
# ── bridge OFF; wrong-garment is DEPLOY diagnostic only (OOTD_DEPLOY_WRONG), never in loss ──
